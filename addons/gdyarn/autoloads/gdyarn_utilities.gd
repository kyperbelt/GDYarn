		...
		while lineNumber < fileLines.size():
			...
			while lineNumber < fileLines.size() && fileLines[lineNumber]!="===":
				...
				if should_tag_line(fileLines[lineNumber]) && tag.empty():
					...
					var tagSeed = (33 * lineNumber * OS.get_time().second + source_key.hash() + fileLines[lineNumber].hash()) % 65537
	...
	for source_key in sources:
		...
		while lineNumber < fileLines.size():
			...
			while lineNumber < fileLines.size() && fileLines[lineNumber]!="===":
				var tag = get_line_tag(fileLines[lineNumber])
				if lineTags.has(tag):
					return {"error" : "duplicate line tag[%s] in file[%s] and file[%s]" % [tag,source_key,lineTags[tag]]}
				if !tag.empty():
...
static func should_tag_line(line : String)->bool:
	...
	if commandStartRegex.search(line.strip_edges()) || !commentTrimRegex.search(line) || commentTrimRegex.search(line).get_string().empty():
		return false
	return true
